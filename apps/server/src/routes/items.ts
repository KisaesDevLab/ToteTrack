...
import { mapItem, requireLiveBox } from '../services/boxes.js';
  ...
  r.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      const row = await db.transaction(async (tx) => {
        const [existing] = await tx.select().from(items).where(eq(items.id, id)).limit(1);
        if (!existing) throw notFound('Item not found');
        // Moving to another box: the photo link stays behind (the photo belongs to the old box).
        const moving = input.boxId !== undefined && input.boxId !== existing.boxId;
        if (moving) await requireLiveBox(tx, input.boxId!);
        const [updated] = await tx
          ...
          .set({
            ...
            ...(moving ? { boxId: input.boxId!, photoId: null } : {}),
            updatedAt: sql`now()` as unknown as Date,
          })
          .where(eq(items.id, id))
          .returning();
        await refreshBoxSearchVector(tx, updated!.boxId);
        if (moving) await refreshBoxSearchVector(tx, existing.boxId);
        return updated!;
      });
