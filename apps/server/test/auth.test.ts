...
describe('auth', () => {
  ...
  it('requires setup first, rejects protected routes, then allows setup + login', async () => {
    ...
    await ctx.agent.post('/api/auth/setup').send({ pin: 'other' }).expect(409);
    ...
    await ctx.agent.post('/api/auth/login').send({ pin: 'wrong' }).expect(401);
  ...
  it('rate limits failed logins to 5 per 15 minutes per IP', async () => {
  ...
  it('changing the PIN keeps other sessions logged in and requires the new PIN afterwards', async () => {
    ...
    await ctx.agent
      ...
      .send({ currentPin: 'wrong', newPin: '9999' })
      .expect(401);
    await ctx.agent
      .post('/api/auth/change-pin')
  ...
  it('rotating the session generation signs out every device', async () => {
    ...
    await ctx.agent
      .post('/api/auth/login')
      .set('CF-Connecting-IP', '198.51.100.3')
      .send({ pin: '9999' })
      .expect(200);
    await ctx.agent.get('/api/boxes').expect(200);
  });
