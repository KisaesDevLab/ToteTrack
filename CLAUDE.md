...
- **Pre-printed labels** (`services/preprint.ts`, table `preprinted_labels`): `reservePreprinted` takes the next N numbers of a series and bumps `next_number`; `lookupLabel` powers `/b/:labelId` (box → open; unclaimed pre-print → auto-create with that number + `?capture=1`; unknown → offer create); `createBox` calls `claimPreprinted` (also sets `printed_at`). **Rescan** = `POST /api/boxes/:id/rescan` (multipart, `replace=true` default): `clearBoxPhotos` drops photos + AI items, stores new photos, then one box-level `ai.enqueueBox`. UI: `components/ScanPanel.tsx`, `components/PreprintCard.tsx`.
