...
- **Pre-printed labels** (`services/preprint.ts`, table `preprinted_labels`): `reservePreprinted` takes the next N numbers of a series and bumps `next_number`; `lookupLabel` powers `/b/:labelId` (box → open; unclaimed pre-print → auto-create with that number + `?capture=1`; unknown → offer create); `createBox` calls `claimPreprinted` (also sets `printed_at`). **Rescan** = `POST /api/boxes/:id/rescan` (multipart, `replace=true` default): `clearBoxPhotos` drops photos + AI items, stores new photos, then one box-level `ai.enqueueBox`. UI: `components/ScanPanel.tsx` (multi-shot queue → one rescan upload), `components/PreprintCard.tsx`, `pages/ScanPage.tsx` (in-app QR reader: BarcodeDetector → jsQR fallback). Setting `scan_opens_camera` (default on) makes `/b/:labelId` land on `?capture=1`, which opens the panel in capture (no photos) or rescan mode.
...
- **Trash / soft delete**: `boxes.deleted_at` / `photos.deleted_at`. Every list/search/count/export/lookup query must filter with `liveBoxes` (services/boxes.ts) or `deleted_at IS NULL` in raw SQL; `getBoxSummary`/`getBoxDetail` deliberately return trashed boxes (UI shows the Trash banner). `DELETE /boxes/:id` and `/photos/:id` soft-delete unless `?permanent=true`; `services/trash.ts` lists and purges (`schedulePurge` in index.ts, 30 days). Rescan moves replaced photos to the Trash instead of deleting files.
- **Backup** (`services/backup.ts`): streamed zip (manifest + `data/<table>.json` + `photos/<path>`), restore is disk-staged (multer diskStorage) → transactional truncate+insert with ids preserved → photo files swapped in after commit. `OMITTED_SETTINGS` (secrets + PIN) never leave via the API — keep it that way.
