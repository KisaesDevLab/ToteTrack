...
- **Env** (`env.ts`): zod-validated; loads the nearest `.env` (dev/test only). `PUBLIC_URL` is the default QR origin (the `public_url` setting overrides it — use `effectivePublicUrl`) and decides `Secure` cookies; `sessionCookieSecure` is only true for https in production.
