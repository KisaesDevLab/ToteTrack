...
- **AI** (`services/ai.ts`): `AiService` = in-process FIFO queue (concurrency 1) with `ai_status` columns; `recoverPending()` on boot re-queues. Prompt asks for JSON `{description, items:[{name,qty,note}]}`; `parseAnalysis` strips fences / extracts the outer object / falls back to description-only. Per-photo run replaces that photo's `source='ai'` rows and sets `photos.ai_description`; box-level run sends all photos in one request and replaces all AI rows. Disabled entirely without `ANTHROPIC_API_KEY`; auto-analyze toggle in settings. Tests inject `analyzeOverride` instead of calling the API.
...
- **Env** (`env.ts`): zod-validated; loads the nearest `.env` (dev/test only). `PUBLIC_URL` feeds QR payloads and decides `Secure` cookies; `sessionCookieSecure` is only true for https in production.
